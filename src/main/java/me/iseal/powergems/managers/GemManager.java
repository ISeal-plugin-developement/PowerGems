...
/**
 * This class is responsible for managing the creation, identification, and usage of "gems" in the game.
 * A gem is represented as an ItemStack with specific metadata.
 * Each gem has a unique power and level associated with it.
 * The class provides methods to create gems, check if an item is a gem, get a random gem, and more.
 */
public class GemManager {
    ...
    public ItemStack createGem(){
        ...
        if (repeating >= cm.getGemCreationAttempts()){
            Bukkit.getLogger().warning("Could not find a gem to create, either you got extremely unlucky or you have too many gems disabled.");
            return null;
        }
     ...
     * If the configuration allows randomized colors, a random color is chosen from the possible colors.
     ...
     * @return An ArrayList of ItemStacks representing the gems in the player's inventory.
     */
     ...
     * @param item The ItemStack to check.
     * @return The Class object representing the gem's class, or null if the item is not a gem.
     */
